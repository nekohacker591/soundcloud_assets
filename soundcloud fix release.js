...
// @version      2.8
// @description  fix soundcloud - performance improvements
...
(function() {
    ...
    let debounceTimerPlayer;
    const DEBOUNCE_DELAY = 50; // ms

    function scheduleModifyPlayerSVGs() {
        clearTimeout(debounceTimerPlayer);
        debounceTimerPlayer = setTimeout(modifyPlayerSVGs, DEBOUNCE_DELAY);
    }

    function modifyPlayerSVGs() {
        const playerControls = document.querySelector('.playControls__elements');
        if (!playerControls) return;

        const buttonsToModify = [
            playerControls.querySelector('.playControl.playControls__play'),
            playerControls.querySelector('.skipControl.playControls__prev'),
            playerControls.querySelector('.skipControl.playControls__next')
        ];
    ...
    let initAttempts = 0;
    const MAX_INIT_ATTEMPTS = 15; // Try for up to 15 seconds
    ...
    function initSVGModification() {
        ...
        if (controlsObserverTarget && !playerObserver) {
            playerObserver = new MutationObserver((mutationsList, obs) => {
                let triggerModification = false;
                for (const mutation of mutationsList) {
                    if (mutation.type === 'childList') {
                         // If any added/removed node is an SVG, or contains an SVG, or is a control button itself
                        for (const node of Array.from(mutation.addedNodes).concat(Array.from(mutation.removedNodes))) {
                            if (node.nodeType === 1 && (node.matches('svg, .playControl, .skipControl') || node.querySelector('svg'))) {
                                triggerModification = true;
                                break;
                            }
                        }
                    } else if (mutation.type === 'attributes') {
                        // If attributes change on SVGs/paths within controls, or on the controls themselves
                        const target = mutation.target;
                         if (target.nodeType === 1 && (target.matches('svg, svg *, .playControl, .skipControl') || target.closest('.playControl, .skipControl'))) {
                            triggerModification = true;
                        }
                    }
                    if (triggerModification) break;
                }

                if (triggerModification) {
                    scheduleModifyPlayerSVGs();
                }
            });
            playerObserver.observe(controlsObserverTarget, { childList: true, subtree: true, attributes: true, attributeFilter: ['class', 'd', 'fill', 'style'] });
        }

        if (headerObserverTarget && !logoObserver) {
            logoObserver = new MutationObserver((mutationsList, obs) => {
                for (const mutation of mutationsList) {
                    let relevantMutation = false;
                     if (mutation.target.matches('.header__logoLink-wordmark div, .header__logoLink-wordmark div *')) {
                        relevantMutation = true;
                    } else if (mutation.type === 'childList') {
                        for (const node of Array.from(mutation.addedNodes).concat(Array.from(mutation.removedNodes))) {
                            if (node.nodeType === 1 && (node.matches('.header__logoLink-wordmark div') || node.closest('.header__logoLink-wordmark div') || node.querySelector('svg'))) {
                                relevantMutation = true;
                                break;
                            }
                        }
        ...
        initAttempts++;
        if ((!controlsObserverTarget || !headerObserverTarget || !playerObserver || !logoObserver) && initAttempts < MAX_INIT_ATTEMPTS) {
